package com.zhouhang.ohsay.view.recyclerview;
...
import com.zhouhang.ohsay.mvp.bean.FoodHeadEntity;
...
public class FloatingItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private Map<Integer, FoodHeadEntity> keys = new HashMap<>();
...
//                Log.e(TAG, "onDrawOver: "+child.getTop()+"$"+firstVisiblePos );
    ...
    private String getTitle(int position) {
        while (position >= 0) {
            if (keys.containsKey(position)) {
                return keys.get(position).getName();
            }
            position--;
        }
        return null;
    ...
    public void setKeys(Map<Integer, FoodHeadEntity> keys) {
        this.keys.clear();
...
//                Log.e(TAG, "drawVertical: "+bottom );
                c.drawText(keys.get(params.getViewLayoutPosition()).getName(), x, y, mTextPaint);

                c.drawText(keys.get(params.getViewLayoutPosition()).getId(), x + 260, y, mTextPaint);
            }
        }
    }
